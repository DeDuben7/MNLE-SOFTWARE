...
void MIDI_PROC(uint8_t MIDI_MSG)
{
	...
	int err,i = 0; // error variabele

	if(MIDI_MSG & 128)
	{
		MIDI_FUNC = (char)(MIDI_MSG & 0x0F); // lsb bits van byte omzetten in nibble
		MIDI_CHANNEL = (char)((MIDI_MSG & 0x0F)>>4); // msb bits van byte omzetten in nibble
	}
